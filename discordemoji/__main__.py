...
class MyClient(discord.Client):
    ...
    async def get_emoji_results(self, days:int, guild: discord.Guild):
        ...
        print(f"{days} days, {total_msgs} messages, {total_sent} emoji sent, {total_reacted} emoji reacted.")
...
class DayResults:
    ...
    def try_cache(on_date : date, guild: discord.Guild) -> Optional["DayResults"]:
        ...
        if os.path.exists(filename):
            with open(filename, mode='rb') as fd:
                obj = pickle.load(fd)
                if type(obj) is DayResults and obj.day == on_date and obj.guild_id == guild.id:
    ...
    async def count_emoji(on_date : date, guild : discord.Guild) -> "DayResults":
        all_channels = aiostream.stream.merge(
            *[chan.history(
            ...
            for chan in guild.text_channels if guild.me in chan.members]
        )
    ...
    def __str__(self) -> str:
        return (f"<{self.day.isoformat()}: {self.message_count} msgs, "
...
def find_custom_emoji(m: discord.Message) -> Generator[discord.PartialEmoji, None, None]:
